...
impl BlockCompressor for LZ4Compressor {
    ...
    fn decompress_block(&self, compressed_data: &[u8], uncompressed_size: usize, buffer: &mut [u8]) {
        let _ = block::decompress_to_buffer(compressed_data, Some(uncompressed_size as i32), buffer);
    }
    ...
    fn decompress_block_to_cache(&mut self, block_index: usize) {
        ...
        unsafe {
            ...
            let _ = block::decompress_to_buffer(compressed_data, Some(uncompressed_size as i32), &mut self.cache);
        }
