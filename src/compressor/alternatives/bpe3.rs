// BPE v3: saves continuation bits in a separate bitvector to decode 8 bits in parallel using SIMD instructions. Dictionary entries have a maximum length of 16 bytes.

use super::Compressor;
use crate::bit_vector::BitVector;
use crate::vbe_simd::{vbe_encode, vbe_decode_simd};
use std::arch::x86_64::*;
...
impl Compressor for BPECompressor {
    fn new(data_size: usize, n_elements: usize) -> Self {
        BPECompressor {
            ...
            continuation_bits: BitVector::new(),
    ...
    fn compress(&mut self, data: &[u8], end_positions: &[usize]) {
        ...
        for end in compressed_strings_separators {
            for &token_id in compressed_strings[start..end].iter() {
                vbe_encode(token_id, &mut self.data, &mut self.continuation_bits);
            } 
            self.item_end_positions.push(self.continuation_bits.len());
            start = end;
        }
    }

    fn decompress(&self, buffer: &mut Vec<u8>) {
        let mut tokens = [0u16; 8];
        let mut pos = 0;

        let data_ptr = self.data.as_ptr();
        let cbits_ptr = self.continuation_bits.data.as_ptr() as *const u8;
        let dict_ptr = self.dictionary.as_ptr();
        let end_positions_ptr = self.dictionary_end_positions.as_ptr();
        let tokens_ptr = tokens.as_ptr();

        let n_iterations = self.continuation_bits.len() / 8;

        // Decode batches of 8 token IDs in parallel using SIMD instructions
        unsafe {
            for iter in 0..n_iterations {
                let length = vbe_decode_simd(data_ptr.add(pos), *cbits_ptr.add(iter), &mut tokens);
                pos += length;
        ...
        // Process remainining continuation bits (if any)
        unsafe {
            for i in n_iterations*8..self.continuation_bits.len() {
                let is_double_bytes = self.continuation_bits.get(i).unwrap();
                let token_id = if is_double_bytes {
                    // Combine two bytes into `token_id`
                    *data_ptr.add(pos) as usize | ((*data_ptr.add(pos + 1) as usize) << 8)
                } else {
                    *data_ptr.add(pos) as usize
                };
                pos += 1 + is_double_bytes as usize;
    ...
    fn space_used_bytes(&self) -> usize {
        self.data.len() + self.continuation_bits.len() / 8 + self.dictionary.len() + (self.dictionary_end_positions.len() * 4)
    }
